...
class Sales extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		if($_GET['customerId']){
            $customer_openint=$this->db->query("select opening_balance from db_customers where id=".$_GET['customerId']."")->row()->opening_balance;
            $customer_openint=$customer_openint>0?$customer_openint:0;

            $row = array();
            $row[] = '';
            $row[] = show_date(date('Y-m-d'));

...
class HomeGallery_model extends CI_Model {
	...
	var $column_search = array('title','link','short_description','image','order_by'); //set column field database for datatable searchable
	...
	public function verify_and_save(){
        ...
        if(!empty($_FILES['image']['name'])){
            ...
            $config['upload_path']          = './uploads/gallery_image/';
            ...
            if ( ! $this->upload->do_upload('image')){
                ...
                $config['source_image'] = 'uploads/gallery_image/'.$file_name;
        ...
        $query1="insert into home_gallerys(image,title,short_description,link)
	...
	public function get_details($id,$data){
		//Validate This category already exist or not
		$query=$this->db->query("select * from home_gallerys where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
	...
	public function update_gallery(){
        ...
        if(!empty($_FILES['image']['name'])){
			...
			$query1="update home_gallerys set $file_name short_description='$short_description',title='$title',link='$link' where id=$q_id";
			if ($this->db->simple_query($query1)){
	...
	public function delete_slider_from_table($ids){
			...
			$query1="delete from home_gallerys where id in($ids)";
	        if ($this->db->simple_query($query1)){
	            echo "success";
	        }
	        else{

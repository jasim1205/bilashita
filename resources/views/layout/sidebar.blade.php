...
.sidebar-menu-container {
  ...
  overflow-y: scroll; /* Always show scrollbar for a more stable render */
  padding-right: 15px; /* Padding to avoid content clipping */
}
</style>
  ...
  <div class="sidebar-menu-container">
    <div class="navbar ">
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item {{$cat->sub_category->count()>0?'dropdown':''}}">
            ...
            <ul class="dropdown-menu  py-0">
              @foreach ($cat->sub_category as $subcat)
                <li>
                  ...
                  @else
                  <li class="nav-item">
                    <a class="nav-link subcategorybutton subcat{{$subcat->id}}" href="{{ route('category.product.list',[$cat->id,$subcat->id]) }}">{{ $subcat->subcategory_name }}</a>
                  </li>
                  @endif
                </li>
              @endforeach
          ...
          @else
          <li class="nav-item">
            <a class="nav-link categorybutton cat{{$cat->id}}" href="{{ route('category.product.list',[$cat->id]) }}">{{ $cat->category_name }}</a>
          </li>
          @endif
        </li>
        @empty
